package verdantium.demo.mycontainerapp.help;
...
import verdantium.demo.mycontainerapp.ScrollingMyContainerApp;
...
/**
 * Online help class for ScrollingMyContainerApp.
 * 
 * @author tgreen
 *
 */
public class ScrollingMyContainerAppHelp
	extends WrappingScrollingVerdantiumHelp {
	
	private ScrollingMyContainerApp MyCell = new ScrollingMyContainerApp();
     ...
     */
	public ScrollingMyContainerAppHelp(URL in) {
		super(in);
		...
		BkEdit =
			MyEdit3.getBackgroundEditor().getBackCell().makePropertiesEditor();
	...
	public static void run(VerdantiumComponent in) {
		URL u =
			ScrollingMyContainerAppHelp.class.getResource(
				"ScrollingMyContainerAppHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ScrollingMyContainerAppHelp hlp = new ScrollingMyContainerAppHelp(u);
		ProgramDirector.showPropertyEditor(
			hlp,
			frm,
			"Scrolling My Container App Help");
		hlp.parseElements();
	}
	...
	protected void dispatchAction(String txt) {
		if (txt.equals("comp")) {
			ScrollingMyContainerApp MyC = MyCell;
			insertComponent(MyC);
		}
