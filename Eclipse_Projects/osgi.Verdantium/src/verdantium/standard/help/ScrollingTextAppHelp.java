...
import verdantium.standard.ScrollingTextApp;
...
/**
 * Online help class for scrolling text app.
 * 
 * @author tgreen
 *
 */
public class ScrollingTextAppHelp extends WrappingScrollingVerdantiumHelp {
	
	private ScrollingTextApp MyCell = new ScrollingTextApp();
     ...
     */
	public ScrollingTextAppHelp(URL in) {
		super(in);
	...
	public static void run(VerdantiumComponent in) {
		URL u = ScrollingTextAppHelp.class.getResource("ScrollingTextAppHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ScrollingTextAppHelp hlp = new ScrollingTextAppHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "Scrolling Text App Help");
		hlp.parseElements();
	}
