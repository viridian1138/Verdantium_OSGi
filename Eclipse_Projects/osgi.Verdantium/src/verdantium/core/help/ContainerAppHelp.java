...
/**
 * Online help class for ContainerApp.
 * 
 * @author tgreen
 *
 */
public class ContainerAppHelp extends WrappingScrollingVerdantiumHelp {
	private ContainerApp MyCell = new ContainerApp();
     ...
     */
	public ContainerAppHelp(URL in) {
		super(in);
	...
	public static void run(VerdantiumComponent in) {
		URL u = ContainerAppHelp.class.getResource("ContainerAppHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ContainerAppHelp hlp = new ContainerAppHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "Container App Help");
		hlp.parseElements();
	}
