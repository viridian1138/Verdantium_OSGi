...
import verdantium.standard.DrawApp;
...
/**
 * Online help class for draw app.
 * 
 * @author tgreen
 *
 */
public class DrawAppHelp extends WrappingScrollingVerdantiumHelp {
	
	private DrawApp MyCell = new DrawApp();
	...
	public static void run(VerdantiumComponent in) {
		URL u = DrawAppHelp.class.getResource("DrawAppHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		DrawAppHelp hlp = new DrawAppHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "Draw App Help");
		hlp.parseElements();
	}
