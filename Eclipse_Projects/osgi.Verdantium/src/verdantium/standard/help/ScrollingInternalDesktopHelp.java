...
/**
 * Online help class for scrolling internal desktop.
 * 
 * @author tgreen
 *
 */
public class ScrollingInternalDesktopHelp
	extends WrappingScrollingVerdantiumHelp {
	
	private ScrollingInternalDesktop MyCell = new ScrollingInternalDesktop();
     ...
     */
	public ScrollingInternalDesktopHelp(URL in) {
		super(in);
	...
	public static void run(VerdantiumComponent in) {
		URL u =
			ScrollingInternalDesktopHelp.class.getResource(
				"ScrollingInternalDesktopHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ScrollingInternalDesktopHelp hlp = new ScrollingInternalDesktopHelp(u);
		ProgramDirector.showPropertyEditor(
			hlp,
			frm,
			"Scrolling Internal Desktop Help");
		hlp.parseElements();
	}
