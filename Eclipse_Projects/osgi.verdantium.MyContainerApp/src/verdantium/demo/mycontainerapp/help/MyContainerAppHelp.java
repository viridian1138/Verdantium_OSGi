...
import verdantium.demo.mycontainerapp.MyContainerApp;
...
/**
 * Online help class for MyContainerApp.
 * 
 * @author tgreen
 *
 */
public class MyContainerAppHelp extends WrappingScrollingVerdantiumHelp {
	
	private MyContainerApp MyCell = new MyContainerApp();
	...
	public static void run(VerdantiumComponent in) {
		URL u =
			MyContainerAppHelp.class.getResource("MyContainerAppHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		MyContainerAppHelp hlp = new MyContainerAppHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "My Container App Help");
		hlp.parseElements();
	}
