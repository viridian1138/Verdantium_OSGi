...
import verdantium.pagewelder.ScrollingPageWelder;
...
/**
 * Online help class for ScrollingPageWelder.
 * 
 * @author tgreen
 *
 */
public class ScrollingPageWelderHelp extends WrappingScrollingVerdantiumHelp {
	
	private ScrollingPageWelder MyCell = new ScrollingPageWelder();
     ...
     */
	public ScrollingPageWelderHelp(URL in) {
		super(in);
	...
	public static void run(VerdantiumComponent in) {
		URL u =
			ScrollingPageWelderHelp.class.getResource(
				"ScrollingPageWelderHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ScrollingPageWelderHelp hlp = new ScrollingPageWelderHelp(u);
		ProgramDirector.showPropertyEditor(
			hlp,
			frm,
			"Scrolling Page Welder Help");
		hlp.parseElements();
	}
	...
	protected void dispatchAction(String txt) {
		if (txt.equals("comp")) {
			ScrollingPageWelder MyC = MyCell;
			insertComponent(MyC);
		}
