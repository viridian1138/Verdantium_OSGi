...
import verdantium.standard.ImageViewer;
...
/**
 * Online help class for image viewer.
 * 
 * @author tgreen
 *
 */
public class ImageViewerHelp extends WrappingScrollingVerdantiumHelp {
	
	private ImageViewer MyCell = new ImageViewer();
	...
	public static void run(VerdantiumComponent in) {
		URL u = ImageViewerHelp.class.getResource("ImageViewerHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ImageViewerHelp hlp = new ImageViewerHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "Image Viewer Help");
		hlp.parseElements();
	}
