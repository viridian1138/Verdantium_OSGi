...
import verdantium.standard.TextApp;
...
/**
 * Online help class for text app.
 * 
 * @author tgreen
 *
 */
public class TextAppHelp extends WrappingScrollingVerdantiumHelp {
	
	private TextApp MyCell = new TextApp();
	...
	public static void run(VerdantiumComponent in) {
		URL u = TextAppHelp.class.getResource("TextAppHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		TextAppHelp hlp = new TextAppHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "Text App Help");
		hlp.parseElements();
	}
