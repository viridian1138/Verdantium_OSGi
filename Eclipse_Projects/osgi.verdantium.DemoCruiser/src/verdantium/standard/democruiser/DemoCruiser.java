...
import verdantium.standard.democruiser.help.DemoCruiserHelp;
...
/**
 * DemoCruiser is a component that facilitates appealing demos.
 * 
 * @author Thorn Green
 */
public class DemoCruiser extends ApplicationAdapter implements
		BackgroundListener, MouseListener {
	 ...
	 */
	protected DemoCruiserDesktopPane myDesk = null;
	...
	public DemoCruiser() {
		...
		UndoManager mgr = UndoManager
				.createInstanceUndoManager(jundo.runtime.Runtime
						.getInitialMilieu());
		undoMgr = mgr;
		initialize(mgr);
		editorBridge = new EditorUndoBridge(undoMgr);
		myDesk = new DemoCruiserDesktopPane(mgr, this);
	...
	*/
	public static DataFlavor[] getPersistentInputDataFlavorsSupported() {
		DataFlavor[] MyF = { new TransVersionBufferFlavor("Demo Cruiser",
				"Demo Cruiser") };
		return (MyF);
	}
	...
	*/
	public void loadPersistentData(DataFlavor flavor, Transferable trans)
			...
			ResourceNotFoundException, ComponentNotFoundException {
		...
		if (trans == null) {
			...
			try {
				...
				ContainerApp.loadInternalDesktopFrames(MyT,
						ContainerAppInternalFrame.class, undoMgr, myDesk);
	...
	*/
	public Transferable savePersistentData(DataFlavor flavor)
			throws IOException {
		TransVersionBuffer MyF = new TransVersionBuffer("Demo Cruiser",
				"Demo Cruiser");

		try {
			MyF.setProperty("Frames", ContainerApp
					.saveInternalDesktopFrames(myDesk));
			MyF.setBoolean("OnlyDesignerEdits", isOnlyDesignerEdits());
			bkgnd.writeData(MyF);
			macroMap.writeData(MyF);
			MyF.setProperty("PageSize", new Dimension(myPan.getBounds().width,
	...
	*/
	public static void main(String argv[]) {
		...
		DemoCruiser MyComp = new DemoCruiser();
		ProgramDirector.showComponent(MyComp, "Demo Cruiser", argv, true);
	}
