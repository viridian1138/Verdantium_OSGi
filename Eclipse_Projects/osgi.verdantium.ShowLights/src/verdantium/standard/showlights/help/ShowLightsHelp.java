package verdantium.standard.showlights.help;
...
import verdantium.standard.showlights.ShowLights;
import verdantium.standard.showlights.ShowLightsPropertyEditor;
 ...
 * 
 * Please document all changes to this source file here. Feel free to add rows
 * if needed.
 * 
 * 
 * |-----------------------|-------------------------------------------------|----------------------------------------------------------------------|---------------------------------------------------------------... |
 * Date of Modification | Author of Modification | Reason for Modification |
 * Description of Modification (use multiple rows if needed) ...
 * |-----------------------|-------------------------------------------------|----------------------------------------------------------------------|---------------------------------------------------------------... | | | | |
 * 9/24/2000 | Thorn Green (viridian_1138@yahoo.com) | Needed to provide a
 * standard way to document source file changes. | Added a souce modification
 * list to the documentation so that changes to the souce could be recorded. |
 * 10/22/2000 | Thorn Green (viridian_1138@yahoo.com) | Methods did not have
 * names that followed standard Java conventions. | Performed a global
 * modification to bring the names within spec. | 06/24/2001 | Thorn Green
 * (viridian_1138@yahoo.com) | Macro support. | Added documentation for macro
 * support. | 08/07/2004 | Thorn Green (viridian_1138@yahoo.com) | Establish
 * baseline for all changes in the last year. | Establish baseline for all
...
/**
 * Online help class for ShowLights.
 * 
 * @author tgreen
 *
 */
public class ShowLightsHelp extends WrappingScrollingVerdantiumHelp {
	private ShowLights MyCell = new ShowLights();
	...
	public ShowLightsHelp(URL in) {
		...
		try {
			...
			TopEdit = ((ShowLightsPropertyEditor) MyEdit0).getTopColorCell()
	...
	public static void run(VerdantiumComponent in) {
		URL u = ShowLightsHelp.class.getResource("ShowLightsHelpPage.rtf");
		JComponent frm = null;
		if (in != null)
			frm = in.getGUI();
		ShowLightsHelp hlp = new ShowLightsHelp(u);
		ProgramDirector.showPropertyEditor(hlp, frm, "ShowLights Help");
		hlp.parseElements();
	}
